/*
 * test_stack_update_array.cpp
 *
 *  Created on: Feb 14, 2013
 *      Author: ipopa
 */
#include <iostream>
#include <cstring>

#include "test_client_common.h"

using namespace std;

static const D_UINT16 MAX_VALUES_ENTRIES = 10;

struct ArrayValuesEntry
{
  D_UINT16      type;
  D_UINT16      elementsCount;
  const D_CHAR* values[MAX_VALUES_ENTRIES];
};

struct ArrayValuesEntry _values[] =
    {
        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},

        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},

        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},


        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},


        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},


        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},


        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},



        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},


        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},

        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},

        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},

        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }},


        /* Repeat */

        {WFT_BOOL, 5, {"0", "0", "1", "0", "1", }},
        {WFT_CHAR, 5, {"A", "a", "Z", "\xC5\xA2", "\xE2\x8B\xA0", }},
        {WFT_DATE, 3, {"2012/11/21", "2981/3/1", "3123/4/4", }},
        {WFT_DATETIME, 3, {"2012/11/21 14:23:12", "2981/3/1 5:1:1", "3123/8/2 0:1:2", }},
        {WFT_HIRESTIME, 3, {"2012/11/21 14:23:12.001", "2981/3/1 5:1:1.1", "3123/8/2 0:1:2.0004", }},
        {WFT_UINT8, 7, {"127", "128", "254", "0", "1", "255", "123", }},
        {WFT_UINT16, 7, {"32767", "32768", "65534", "0", "1", "65535", "123", }},
        {WFT_UINT32, 7, {"2147483647", "2147483648", "4294967294", "0", "1", "4294967295", "123", }},
        {WFT_UINT64, 7, {"9223372036854775807", "9223372036854775808", "18446744073709551614", "0", "1", "18446744073709551615", "123", }},
        {WFT_REAL, 5, {"-549755813888.999999", "549755813887.999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_RICHREAL, 5, {"-9223372036854775808.99999999999999", "9223372036854775807.99999999999999", "-123.123", "124.012", "-235451.167" }},
        {WFT_INT8, 5, {"-128", "127", "0", "1", "123", }},
        {WFT_INT16, 5, {"-32768", "32767", "0", "1", "123", }},
        {WFT_INT32, 5, {"-2147483648", "2147483647", "0", "1", "123", }},
        {WFT_INT64, 5, {"-9223372036854775808", "9223372036854775807", "0", "1", "123", }},
        {WFT_REAL, 5, {"-549755813887.000001", "-0.000001", "0.000001", "1.1", "0" }},
        {WFT_RICHREAL, 5, {"-9223372036854775807.00000000000001", "-0.00000000000001", "0.00000000000001", "1.1", "0" }}
    };

static const D_UINT _valuesCount  = sizeof (_values) / sizeof (_values[0]);

static bool
fill_array_entries_step (W_CONNECTOR_HND hnd)
{
  for (D_UINT i = 0; i < _valuesCount; i++)
    {
      if ((WPushStackValue (hnd,
                            _values[i].type | WFT_ARRAY_MASK,
                            0,
                            NULL) != WCS_OK)
          || (WUpdateStackFlush (hnd) != WCS_OK))
        {
          return false;
        }

      for (D_UINT j = 0; j < _values[i].elementsCount; ++j)
        {
          if ((WUpdateStackValue (hnd,
                                  _values[i].type,
                                  WIGNORE_FIELD,
                                  WIGNORE_ROW,
                                  j,
                                  WIGNORE_OFF,
                                  _values[i].values[j]) != WCS_OK)
              || (WUpdateStackFlush (hnd) != WCS_OK))
            {
              return false;
            }
        }
    }

  return true;
}

static bool
fill_array_entries_bulk (W_CONNECTOR_HND hnd)
{
  for (D_UINT i = 0; i < _valuesCount; i++)
    {
      if (WPushStackValue (hnd,
                           _values[i].type | WFT_ARRAY_MASK,
                           0,
                           NULL) != WCS_OK)
        {
          return false;
        }

      for (D_UINT j = 0; j < _values[i].elementsCount; ++j)
        {
          if (WUpdateStackValue (hnd,
                                 _values[i].type,
                                 WIGNORE_FIELD,
                                 WIGNORE_ROW,
                                 j,
                                 WIGNORE_OFF,
                                 _values[i].values[j]) != WCS_OK)
            {
              return false;
            }
        }
    }

  if (WUpdateStackFlush (hnd) != WCS_OK)
    return false;

  return true;
}

static bool
check_array_entry (W_CONNECTOR_HND hnd, const D_INT index)
{
  const ArrayValuesEntry* pEntry = &_values[index];
  const D_CHAR*           value;
  unsigned long long int  count;

  if ((WGetStackArrayElementsCount (hnd,
                                    WIGNORE_FIELD,
                                    WIGNORE_ROW,
                                    &count) != WCS_OK)
      || (count != pEntry->elementsCount))
    {
      return false;
    }

  for (D_UINT i = 0; i < pEntry->elementsCount; ++i)
    {
      if ((WGetStackValueEntry (hnd,
                               WIGNORE_FIELD,
                               WIGNORE_ROW,
                               i,
                               WIGNORE_OFF,
                               &value) != WCS_OK)
          || (strcmp (value, pEntry->values[i]) != 0))
        {
          return false;
        }
    }

  if (WGetStackValueEntry (hnd,
                           WIGNORE_FIELD,
                           WIGNORE_ROW,
                           pEntry->elementsCount,
                           WIGNORE_OFF,
                           &value) != WCS_INVALID_ARRAY_OFF)
    {
      return false;
    }

  return true;
}

static bool
test_array_step_update (W_CONNECTOR_HND hnd)
{
  cout << "Testing array step update ... ";

  if (! fill_array_entries_step (hnd))
    goto test_array_step_update_fail;

  for (D_INT i = _valuesCount - 1; i >= 0; i--)
    {
      if (! check_array_entry (hnd, i))
        goto test_array_step_update_fail;

      if ((WPopStackValues (hnd, 1) != WCS_OK)
          || (WUpdateStackFlush (hnd) != WCS_OK))
        {
          goto test_array_step_update_fail;
        }
    }

  cout << "OK\n";
  return true;

test_array_step_update_fail:
  cout << "FAIL\n";
  return false;

}

static bool
test_array_bulk_update (W_CONNECTOR_HND hnd)
{
  cout << "Testing array bulk update ... ";

  if (! fill_array_entries_bulk (hnd))
    goto test_array_bulk_update_fail;

  for (D_INT i = _valuesCount - 1; i >= 0; i--)
    {
      if (! check_array_entry (hnd, i))
        goto test_array_bulk_update_fail;

      if ((WPopStackValues (hnd, 1) != WCS_OK)
          || (WUpdateStackFlush (hnd) != WCS_OK))
        {
          goto test_array_bulk_update_fail;
        }
    }

  cout << "OK\n";
  return true;

test_array_bulk_update_fail:
  cout << "FAIL\n";
  return false;

}

static bool
test_for_errors (W_CONNECTOR_HND hnd)
{
  unsigned long long count;

  cout << "Testing against error conditions ... ";

  if ((WGetStackValueRowsCount (NULL, NULL) != WCS_INVALID_ARGS)
      || (WGetStackValueRowsCount (NULL, &count) != WCS_INVALID_ARGS)
      || (WGetStackValueRowsCount (hnd, NULL) != WCS_INVALID_ARGS))
    {
      goto test_for_errors_fail;
    }

  if ((WPushStackValue (hnd, WFT_BOOL, 0, NULL) != WCS_OK)
      || (WUpdateStackFlush (hnd) != WCS_OK)
      || (WGetStackArrayElementsCount (hnd, WIGNORE_FIELD, WIGNORE_ROW, &count) != WCS_INVALID_ARGS)
      || (WPushStackValue (hnd, WFT_ARRAY_MASK | WFT_BOOL, 0, NULL) != WCS_OK)
      || (WUpdateStackFlush (hnd) != WCS_OK)
      || (WGetStackArrayElementsCount (hnd, "somef", WIGNORE_ROW, &count) != WCS_INVALID_FIELD)
      || (WGetStackArrayElementsCount (hnd, WIGNORE_FIELD, 0, &count) != WCS_INVALID_ROW)
      || (WGetStackArrayElementsCount (hnd, WIGNORE_FIELD, WIGNORE_ROW, NULL) != WCS_INVALID_ARGS)
      || (WGetStackArrayElementsCount (hnd, WIGNORE_FIELD, WIGNORE_ROW, &count) != WCS_INVALID_ARGS)
      || (WPopStackValues (hnd, WPOP_ALL) != WCS_OK)
      || (WUpdateStackFlush (hnd) != WCS_OK))
    {
      goto test_for_errors_fail;
    }

  cout << "OK\n";
  return true;

test_for_errors_fail :
  cout << "FAIL\n";
  return false;
}


const D_CHAR*
DefaultDatabaseName ()
{
  return "test_list_db";
}

const D_UINT
DefaultUserId ()
{
  return 1;
}

const D_CHAR*
DefaultUserPassword ()
{
  return "test_password";
}

int
main (int argc, const char** argv)
{
  W_CONNECTOR_HND hnd        = NULL;

  bool success = tc_settup_connection (argc, argv, &hnd);

  success = success && test_for_errors (hnd);
  success = success && test_array_step_update (hnd);
  success = success && test_array_bulk_update (hnd);

  WClose (hnd);

  if (!success)
    {
      cout << "TEST RESULT: FAIL" << std::endl;
      return 1;
    }

  cout << "TEST RESULT: PASS" << std::endl;

  return 0;
}
