...
data_size(const struct INTERNAL_HANDLER* const hnd)
{
  ...
  assert(metaDataSize <= frameSize && frameSize <= hnd->dataSize);
...
static void
set_data_size(struct INTERNAL_HANDLER* const   hnd,
              const uint_t                     size)
{
  uint_t metaDataSize = FRAME_HDR_SIZE + PLAIN_HDR_SIZE;
                ...
                const uint8_t                   type)
{
  ...
  if (hnd->cipher == FRAME_ENCTYPE_3K)
  {
    ...
    store_le_int32(firstKing, hnd->data + FRAME_HDR_SIZE + ENC_3K_FIRST_KING_OFF);

    secondKing = wh_rnd() & 0xFFFFFFFF;
    store_le_int32(secondKing, hnd->data + FRAME_HDR_SIZE + ENC_3K_SECOND_KING_OFF);

    for (i = 0, prev = 0; i < ENC_PLAIN_SIZE_OFF; ++i)
    {
      const uint8_t t = hnd->data[FRAME_HDR_SIZE + i];
      hnd->data[FRAME_HDR_SIZE + i] ^= hnd->keys._3K[prev % keySize];
      prev = t;
    }

    store_le_int16(plainSize, hnd->data + FRAME_HDR_SIZE + ENC_PLAIN_SIZE_OFF);
    store_le_int16(wh_rnd(),  hnd->data + FRAME_HDR_SIZE + ENC_SPARE_OFF);
  ...
  else if (hnd->cipher != FRAME_ENCTYPE_PLAIN)
  {
    ...
    if (hnd->cipher == FRAME_ENCTYPE_DES)
      wh_buff_des_encode_ex(hnd->keys._DES, hnd->data + FRAME_HDR_SIZE, frameSize - FRAME_HDR_SIZE);
...
receive_raw_frame(struct INTERNAL_HANDLER* const hnd)
{
  ...
  while(frameRead < FRAME_HDR_SIZE)
  {
    ...
    const uint32_t status = whs_read(hnd->socket, hnd->data + frameRead, &chunkSize);
    if (status != WOP_OK)
      return WENC_OS_ERROR(status);
  ...
  if (hnd->cipher == FRAME_ENCTYPE_3K)
  {
    ...
    firstKing  = load_le_int32(hnd->data + FRAME_HDR_SIZE + ENC_3K_FIRST_KING_OFF);
    secondKing = load_le_int32(hnd->data + FRAME_HDR_SIZE + ENC_3K_SECOND_KING_OFF);
    ...
    plainSize = load_le_int16(hnd->data + FRAME_HDR_SIZE + ENC_PLAIN_SIZE_OFF);
              ...
              const uint16_t                cmd)
{
  ...
  for (index = 0, chkSum = 0; index < dataSize; index++)
  ...
  return send_raw_frame(hnd, FRAME_TYPE_NORMAL);
}
                ...
                uint16_t* const               outRsp)
{
  ...
  else if (load_le_int32(rawData + PLAIN_CLNT_COOKIE_OFF) != hnd->clientCookie)
  {
         ...
         WH_CONNECTION* const   pHnd)
{
  ...
  uint8_t tempBuffer[MIN_FRAME_SIZE], challenge[sizeof(uint64_t)];
...

  {
    ...
    if (frameId != 0 || result->data[FRAME_TYPE_OFF] != FRAME_TYPE_AUTH_CLNT)
    {
        ...
        && (result->cipher != FRAME_ENCTYPE_3DES))
    {
      ...
      memcpy(result->keys._3K, password, MIN(passwordLen, sizeof result->keys - 1));
    ...
    result->version = load_le_int32(result->data + FRAME_HDR_SIZE + FRAME_AUTH_VER_OFF);
    ...
    serverFrameSize = load_le_int16(result->data + FRAME_HDR_SIZE + FRAME_AUTH_SIZE_OFF);
    ...
    memcpy(challenge, result->data + FRAME_HDR_SIZE + FRAME_AUTH_CHALLENGE_OFF, sizeof challenge);

    result->data     = mem_alloc(serverFrameSize);
    result->dataSize = serverFrameSize;

    store_le_int16(result->dataSize, result->data + FRAME_HDR_SIZE + FRAME_AUTH_RSP_SIZE_OFF);
  }

  {
    ...
    store_le_int32(result->version, &result->data[FRAME_HDR_SIZE + FRAME_AUTH_RSP_VER_OFF]);
    ...
    memcpy(result->data + FRAME_HDR_SIZE + FRAME_AUTH_RSP_CHALLENGE_OFF,
           challenge,
           sizeof challenge);
    strcpy((char*)result->data + FRAME_HDR_SIZE + FRAME_AUTH_RSP_FIXED_SIZE, database);
                  ...
                  uint_t* const         outCount)
{
  ...
  result = list_globals(hnd_, 0, &proxyCount);
                     ...
                     uint_t* const         outCount)
{
  ...
  result = list_procedures(hnd_, 0, &proxyCount);
  if ((result == WCS_OK) && (outCount != NULL))
    *outCount = proxyCount;
               ...
               uint_t* const                    outRawType)
{
  ...
  assert(offset == strlen((char*)data_ + sizeof(uint32_t)) + 1 + sizeof(uint32_t));
...
describe_value_fetch_field_again:
  ...
  else
  {
    ...
    uint_t dummyType;
    cs = describe_value(hnd_, (char*) data(hnd_) + sizeof(uint32_t), fieldHint, &dummyType);
    if (cs != WCS_OK)
      goto describe_value_fetch_field_err;
                        ...
                        const struct WField* const   fields)
{
  ...
  if (type != WHC_TYPE_TABLE_MASK)
  {
    ...
    if (baseType < WHC_TYPE_BOOL
        || WHC_TYPE_TEXT < baseType
        || fieldsCount > 0)
    {
      return 0;
    }
  ...
  while(fieldsCount-- > 0)
  {
    const uint16_t    fieldType = fields[fieldsCount].type & ~WHC_TYPE_ARRAY_MASK;
    const char* const fieldName = fields[fieldsCount].name;
    const uint_t      nameLen   = strlen(fieldName) + 1;

    if (fieldName == NULL
        || nameLen <= 1
        || fieldType < WHC_TYPE_BOOL
        || WHC_TYPE_TEXT < fieldType)
    {
      return 0;
    }
    result += nameLen + sizeof(uint16_t);
  }
...
{
  const uint_t spaceNeed = sizeof(uint8_t) + sizeof(uint16_t) + strlen(value) + 1;
  uint_t       cs        = WCS_OK;
...
stack_top_text_update(struct INTERNAL_HANDLER  *hnd,
                      ...
                      const char* const         value)
{
  const uint_t fixedSize = sizeof(uint8_t) + sizeof(uint16_t) + sizeof(uint64_t);
...
{
  uint_t  cs        = WCS_OK;
  uint_t  countOff  = sizeof(uint8_t) + sizeof(uint16_t) + strlen(fieldName) + 1 + sizeof(uint64_t);
             ...
             const char* const     value)
{
  ...
  if (hnd_ == NULL
      || value == NULL
      || value[0] == 0
      || (type < WHC_TYPE_BOOL) || (WHC_TYPE_TEXT < type))
  {
    return WCS_INVALID_ARGS;
  }
  ...
  else if (row != WIGNORE_ROW)
  {
    ...
    if (arrayOff == WIGNORE_OFF)
    {
      if (type == WHC_TYPE_TEXT)
      {
        ...
        cs = stack_top_field_text_update(hnd_, fieldName, row, textOff, value);
      }
      else
      {
        if (textOff != WIGNORE_OFF)
        ...
        cs = stack_top_field_basic_update(hnd_, type, fieldName, row, value);
      }
    }
    else
    {
      ...
      else
      {
        ...
        cs = stack_top_field_array_basic_update(hnd_, type, fieldName, row, arrayOff, value);
      }
    }
  }
  else
                    ...
                    const uint64_t                   textOff)
{
  ...
  neededSize = strlen(field) + 1 + sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint64_t);
                ...
                ullong_t* const       outCount)
{
  ...
  {
    cs = send_stack_read_req(hnd_, WIGNORE_FIELD, WIGNORE_ROW, WIGNORE_OFF, WIGNORE_OFF);
    if (cs != WCS_OK)
      return cs;
  }
...
resend_req_get_stack_entry:
  ...
  *outType    = load_le_int16(data_ + dataOffset);
  ...
  {
    dataOffset = get_array_el_off(hnd, *outType & 0xFF, arrayOff, dataOffset, outCount);
                ...
                ullong_t* const       outCount)
{
  ...
  cs = get_stack_value(hnd, field, row, 0, WIGNORE_OFF, &count, &type, &value);
                 ...
                 ullong_t* const       outCount)
{
  ...
  cs = get_stack_value(hnd, field, row, arrayOff, 0, &count, &type, &value);
            ...
            const char** const    outpValue)
{
  ...
  uint_t cs = get_stack_value(hnd, field, row, arrayOff, textOff, &count, &type, outpValue);
  if (cs != WCS_OK)
    return cs;
                        ...
                        const uint_t                     param)
{
  ...
  if ((cs = load_le_int32(data_)) != WCS_OK)
    goto describe_proc_parameter_err;
          ...
          || sizeof( uint32_t) + nameLen + 3 * sizeof(uint16_t) > data_size(hnd))
  {
    cs = WCS_INVALID_FRAME;
    goto describe_proc_parameter_err;
  }
                 ...
                 uint_t* const         outCount)
{
  ...
  *outCount = load_le_int16(data_ + sizeof(uint32_t) + strlen(procedure) + 1);
                      ...
                      uint16_t* const                  outOffset)
{
  ...
  *outOffset  = sizeof(uint32_t) + nameLen;
  count       = load_le_int16(data_ + *outOffset);
  *outOffset += sizeof(uint16_t);
  hint        = load_le_int16(data_ + *outOffset);
  *outOffset += sizeof(uint16_t);

  if ((param < hint) || (count < param))
    return WCS_INVALID_ARGS;

  for (it = hint; (it < param) && (*outOffset < data_size(hnd)); ++it)
  {
    const uint16_t type = load_le_int16(data_ + *outOffset);
    ...
    *outOffset += sizeof(uint16_t);
    ...
    if (type & WHC_TYPE_TABLE_MASK)
    {
      ...
      const uint16_t fieldsCount = load_le_int16(data_ + *outOffset);
      *outOffset += sizeof(uint16_t);

      for (field = 0; (field < fieldsCount) && (*outOffset < data_size(hnd)); ++field)
        *outOffset += strlen((char*)data_ + *outOffset) + 1 + sizeof(uint16_t);
    }
    else
      *outOffset += sizeof(uint16_t);
  }

  if ((*outOffset >= data_size(hnd)) || (hint != param))
    return WCS_INVALID_ARGS;
...
proc_parameter_field_count_err:
  ...
  if (tryAgain)
  {
    ...
    if ((cs = describe_proc_parameter(hnd, procedure, param)) == WCS_OK)
...
proc_parameter_field_try_again:
  ...
  for (iterator = 0; iterator < field; ++iterator)
    offset += strlen((char*)data_ + offset) + 1 + sizeof(uint16_t);
