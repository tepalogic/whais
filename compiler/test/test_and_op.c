...
check_procedure (struct ParserState *state, D_CHAR * proc_name)
{
  ...
  if (op_expect != W_AND)
      ...
      if (code_size < 9)
	{
      ...
      else if (w_opcode_decode (code + 9) != op_expect)
	{
	  return FALSE;
	}
      else if (w_opcode_decode (code + 2) != W_JF)
	{
	  return FALSE;
	}
      else if (*(D_INT32 *) (code + 3) != 8)
	{
	  return FALSE;
	}
    }
...
main ()
{
  ...
  printf ("Testing and op symmetry...");
