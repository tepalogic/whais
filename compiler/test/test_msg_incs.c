...
D_CHAR test_prog_1[] = ""
  "PROCEDURE Proc_1 (v2 AS DATE) RETURN DATE\n "
  "DO\n " "RETURN ++v2;\n " "ENDPROC\n ";

D_CHAR test_prog_2[] = ""
  "PROCEDURE Proc_1 (v2 AS REAL) RETURN REAL\n "
  "DO\n " "RETURN ++v2;\n " "ENDPROC\n ";
...
main ()
{
  ...
  test_result = test_for_error (test_prog_1, MSG_INC_NA, MSG_ERROR_EVENT);
  test_result =
    (test_result == FALSE) ? FALSE : test_for_error (test_prog_2, MSG_INC_NA,
						     MSG_ERROR_EVENT);
  test_result =
    (test_result == FALSE) ? FALSE : test_for_error (test_prog_3, MSG_INC_ELV,
						     MSG_ERROR_EVENT);
  test_result =
    (test_result == FALSE) ? FALSE : test_for_error (test_prog_4, MSG_INC_ELV,
						     MSG_ERROR_EVENT);
  test_result =
    (test_result == FALSE) ? FALSE : test_for_error (test_prog_5, MSG_INC_ELV,
						     MSG_ERROR_EVENT);
