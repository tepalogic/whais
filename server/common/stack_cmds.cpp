...
cmd_pop_stack(ClientConnection& conn, uint_t* const inoutDataOff)
{
  ...
  if (( *inoutDataOff + sizeof(uint32_t)) > conn.DataSize())
    throw ConnectionException(_EXTRA(0), "Got frame with invalid content for stack pop operation.");
    ...
    *inoutDataOff += sizeof(uint16_t);
    ...
    for (uint_t field = 0; field < fieldsCount; ++field)
    {
      ...
      *inoutDataOff += fieldLen + 1;
      if (( *inoutDataOff + sizeof(uint16_t) > dataSize) || (fieldLen == 0))
        goto push_frame_error;
      ...
      *inoutDataOff += sizeof(uint16_t);
      ...
      else if (fields_[field].type == WHC_TYPE_TEXT && fields_[field].isArray)
        return WCS_OP_NOTSUPP;
...
push_frame_error:
  throw ConnectionException(_EXTRA(0), "Frame with invalid content for stack push operation.");
           ...
           uint_t* const inoutDataOff)
{
  ...
  {
    DBool value;
    result = Utf8Translator::Read(data + *inoutDataOff, dataSize - *inoutDataOff, &value);
  ...
  {
    DChar value;
    result = Utf8Translator::Read(data + *inoutDataOff, dataSize - *inoutDataOff, false, &value);
  ...
  default:
    throw ConnectionException(_EXTRA(type), "Client frame contains invalid update type.");
    ...
    *inoutDataOff += sizeof(uint16_t);
    if (type & WHC_TYPE_FIELD_MASK)
    {
      ...
      *inoutDataOff += fieldLen;
      if (fieldLen <= 1 || (*inoutDataOff + sizeof(uint64_t)) > dataSize)
        goto update_frame_error;
      ...
      *inoutDataOff += sizeof(uint64_t);
      ...
      if (type & WHC_TYPE_ARRAY_MASK)
      {
        ...
        *inoutDataOff += sizeof(uint64_t);
        ...
        while (( *inoutDataOff < dataSize) && (elCount > 0))
        {
          ...
          if ( ! read_value(arrayValuevalue, type, data, dataSize, inoutDataOff))
            goto update_frame_error;
      ...
      {
        if ( ! read_value(rowValue, type, data, dataSize, inoutDataOff))
          goto update_frame_error;
    ...
    else if (type & WHC_TYPE_ARRAY_MASK)
    {
      ...
      *inoutDataOff += sizeof(uint64_t);
      ...
      while (( *inoutDataOff < dataSize) && (elCount > 0))
      {
        ...
        if ( ! read_value(arrayValue, type, data, dataSize, inoutDataOff))
          goto update_frame_error;
    ...
    else
    {
      ...
      const uint_t elLen = read_value(stackTop, type, data, dataSize, inoutDataOff);
      if (elLen == 0)
        goto update_frame_error;
...
update_frame_error:
  throw ConnectionException(_EXTRA(0), "Frame with invalid content for stack update operation.");
  return WCS_GENERAL_ERR; //Not executed anyway!
}

static uint_t
write_value(StackValue& source, uint8_t* const data, const uint16_t maxDataSize)
{
                         ...
                         uint_t* const inoutDataOffset)
{
  ...
  assert((value.Operand().GetType() >= T_BOOL) && (value.Operand().GetType() < T_TEXT));
  ...
  *inoutDataOffset += nameLen;
  ...
  if (fd.isArray)
  {
    ...
    *inoutDataOffset += sizeof(uint16_t);
    ...
    cs = cmd_read_array_stack_top(conn, rowValue, hintArrayOff, inoutDataOffset);
  }
  else if (fd.type == WHC_TYPE_TEXT)
  {
    ...
    *inoutDataOffset += sizeof(uint16_t);
    ...
    cs = cmd_read_text_stack_top(conn, rowValue, hintTextOff, inoutDataOffset);
  }
  else
  {
    store_le_int16(fd.type, data + *inoutDataOffset);
  ...
  *inoutDataOffset += sizeof(uint64_t);
  ...
  for (uint64_t row = hintRow; row < rowsCount; ++row)
  {
    ...
    if (fieldDesc.isArray)
    {
      ...
      status = cmd_read_array_stack_top(conn, el, hintArrayOff, inoutDataOffset);
    }
    else if (fieldDesc.type == T_TEXT)
    {
      ...
      status = cmd_read_text_stack_top(conn, el, hintTextOff, inoutDataOffset);
    }
    else
    {
      if ((hintArrayOff != WIGNORE_OFF) || (hintTextOff != WIGNORE_OFF))
                         ...
                         uint_t* const     inoutDataOffset)
{
  ...
  {
    const uint_t minSize = sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint16_t);
