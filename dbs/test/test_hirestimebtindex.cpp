...
static DBSHiresTime _max_date (false, 0x7FFF, 12, 31, 23, 59, 59, 999999999);
                        ...
                        DBSArray &tableValues)
{
  ...
  DBSArray values = table.GetMatchingRows (DBSHiresTime (true),
                                           _max_date,
                                           0,
                                           ~0,
                                           0,
  ...
  for (D_UINT64 index = _removedRows; (index < _rowsCount) && result; ++index)
    {
      ...
      DBSHiresTime generatedValue (true);
