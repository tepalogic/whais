...
static DBSRichReal _max_date (false, std::numeric_limits<long double>::infinity ());

DBSRichReal
get_random_real ()
{

  return DBSRichReal (false, _SC (long double, w_rnd () & 0xFFFFFF) / (w_rnd () % (0xFF + 1)) );
}
                        ...
                        DBSArray &tableValues)
{
  ...
  DBSArray values = table.GetMatchingRows (DBSRichReal (true),
                                           _max_date,
                                           0,
                                           ~0,
                                           0,
  ...
  for (D_UINT64 index = _removedRows; (index < _rowsCount) && result; ++index)
    {
      ...
      DBSRichReal generatedValue (true);
